...
int AForm::is_executable(Bureaucrat &person)
{
    if (person.get_grade() > this->get_grade_to_execute())
    {
        throw Bureaucrat::GradeTooLowException();
    }
    if (this->get_signed_state() == false)
    {
        throw Bureaucrat::DocumentIsNotSigned();
    }
    return(1);
}