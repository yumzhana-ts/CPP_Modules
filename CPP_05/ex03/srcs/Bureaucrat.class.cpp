...
{
    return (RED"Error: Document is not signed"RESET_COLOR);
}
...
void Bureaucrat::execute_form(AForm &form)
{
    try
    {
        form.is_executable(*this);
        form.execute();
        std::cout << this->name << " executed " << form.get_name() << std::endl;
    }
    catch(Bureaucrat::DocumentIsNotSigned &e)
    {
        std::cout << this->name << " couldnt execute " << form.get_name() <<  " because of ";
        std::cout << e.what() << std::endl;
    }
    catch(Bureaucrat::GradeTooLowException &e)
    {
